...
2. For listing tasks (when user asks to see, show, list, or view their tasks):
{
    ...
    "data": {
        "filter": {
            ...
            "due": "today|yesterday|overdue|upcoming|YYYY-MM-DD",  // Optional
            "created": "today|YYYY-MM-DD"  // Optional
        }
    }
}
...
Important Notes:
1. For specific dates, always use YYYY-MM-DD format
2. When user mentions a specific date (e.g., "show tasks for January 8th"), 
   set the appropriate filter (due or created) with the date in YYYY-MM-DD format
3. Handle relative dates appropriately (e.g., "today", "yesterday", "tomorrow")
4. For task status, always use one of: "Pending", "In Progress", "Completed"
5. For task priority, always use one of: "High", "Medium", "Low"
"""
